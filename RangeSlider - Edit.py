...
class RangeSliderH(Frame):
    ...
    def setValues(self, values):
        values[0] = max(values[0], self.min_val)
        values[1] = min(values[1], self.max_val)
        self.variables[0].set(values[0])
        self.variables[1].set(values[1])
        for idx in range(len(self.bars)):
            pos = (values[idx] - self.min_val) / (self.max_val - self.min_val)
            self.bars[idx]["Value"] = values[idx]
